...
app.get('/', (req, res) => {
  res.send(`
    ...
    <head>
        ...
        <title>DNG to PNG Converter - API Mode Only</title>
        <style>
            body {
    ...
    <body>
        <div class="container">
            ...
            <div class="mode-badge">API Mode - Server-side Processing Only</div>
            
            <div class="info">
                <strong>Standalone API Mode:</strong><br>
                This server runs only the API mode for server-side DNG to PNG conversion.<br>
                    ...
                    if (response.ok) {
                        ...
                        result.innerHTML = \`
                            <div class="success">
                                ...
                                <a href="/download/\${data.filename}" download style="color: #155724; font-weight: bold;">📥 Download PNG file</a>
                            </div>
                        \`;
                    } else {
...
app.listen(PORT, () => {
  console.log(`🚀 DNG to PNG Converter API Server running on http://localhost:${PORT}`);
  console.log(`🌐 API-only mode: Server-side conversion with dcraw + Sharp`);
});